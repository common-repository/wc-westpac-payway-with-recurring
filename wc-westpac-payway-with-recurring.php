...
add_action( 'woocommerce_blocks_loaded', 'register_paywaynetrecurring_payment_method_type' );
...
*/
function register_paywaynetrecurring_payment_method_type() {
	// Check if the required class exists
	...
	require_once PAYWAYRESTRECURRINGPLUGINPATH . 'includes/class-payway-woocommerce-block-checkout.php';

	// Hook the registration function to the 'woocommerce_blocks_payment_method_type_registration' action
	// add_action(
	// 	'woocommerce_blocks_payment_method_type_registration',
	// 	function( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
	// 		// Register an instance of WC_Payway_Blocks
	// 		$payment_method_registry->register( new WC_Payway_Blocks );
	// 	}
	// );
	...
	add_action(
		...
		function(Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
		  ...
		  $container->register(
			WC_Payway_Blocks::class,
			  function() {
				  return new WC_Payway_Blocks();
			  }
		  );
		  $payment_method_registry->register($container->get(WC_Payway_Blocks::class));
		},
		5
	  );
}
...
function woocommerce_payway_gateway_rest_recurring_init()
{
	...
	**/
	...
	function payway_custom_fields_save( $post_id ){
		...
		//Save No of Payments
		$no_of_payments = sanitize_text_field($_POST['_no_of_payments']);
		...
		//Save Frequency
		$frequency  = sanitize_text_field($_POST['_payway_frequency']);
		if( !empty( $frequency ) )
